...
public class App extends PApplet {
    ...
    public void setup() {
        ...
        duplicateBalls = new LinkedList<>();
        
        ballReleaseInterval = board.getInterval();
        for(int i = 0; i < board.getBalls().size(); i++){
            Ball ball = board.getBalls().get(i);
            duplicateBalls.add(new Ball(ball.colorIndex, (int)ball.position.x, (int)ball.position.y));
        }
    ...
    /**
     * Checks the state of all balls in the game.
     * This method updates the position of each ball and checks for collisions.and handles score
     */
    @SuppressWarnings("rawtypes")
    public void ballCheck() {
        score = 0;  // Reset score for this check
        score += scorePreserver;
        ...
        for (int i = 0; i < ballsReleased; i++) {
                ...
                for (Hole hole : board.holes) {
                    ...
                    if (hole.checkCollision(ball)) {
                        ...
                        if((ball.colorIndex == hole.holeIndex || hole.holeIndex == 0 || ball.colorIndex == 0) && !ball.hit){
                            ...
                            ball.isOut = false;
                            ball.rightHole = true;
                            Ball b = new Ball(ball.colorIndex, (int)ball.position.x, (int)ball.position.y);
                            duplicateBalls.add(b);
                            ball.scoreMinus();
                            ball.hit = !ball.hit;
                        }
            ...
            int offset = 0;  // Tracks how many balls are not in right hole or are out
            // First, count how many balls are not in right hole or are out
            for (int j = 0; j < duplicateBalls.size(); j++) {
                Ball b = duplicateBalls.get(j);
                if (!b.rightHole || b.isOut) {
                    offset++;  // Increment offset for each ball not in right hole or is out
                }
            }
            int jj = 0; // Reset jj to track drawn balls

            // Now, display the balls and shift them to the left by 32 * offset pixels
            for (int j = 0; j < duplicateBalls.size(); j++) {
                Ball b = duplicateBalls.get(j);
                if (b.rightHole && !b.isOut) {
                    b.draw(this, 32 * (jj - offset), TOPBAR / 2);  // Shift the drawing by offset
                    jj++; // Only increment jj if the ball is drawn
                }
            }

            // Now, jj will reflect the number of balls that were drawn successfully
    ...
    /**
     * Determines if all balls have hit their targets.
     * 
     * @return true if all balls have hit their targets, false otherwise.
     */
    ...
    /**
     * Checks if all balls have hit their respective targets.
     * If all balls have hit their targets, it advances to the next level.
     */
    public void checkLevelEnd() {
        if(allBallsHit()){
            double timeBonus = 1 * 15; // 1 unit every 0.067 seconds means 15 units per second
            score += timeBonus;
            scorePreserver += score;
     ...
     * This method clears all game-related data and reinitializes the game setup.
     */
     ...
     * If the current level index is at the last level, it resets to the first level.
     * This method also clears all game-related data and reinitializes the game setup.
     */
     ...
     * This method sets the text size for the game over message.
     */
